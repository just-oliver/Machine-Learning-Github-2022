...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 29 15:14:36 2022
    ...
    while run:
        ...
        for x, body in enumerate(bodies):
            ...
            if body_states[x] == "forward" and delta < 1.17 - 0.05:
                ...
                # Legs
                if 0.5 >= outputs[0] > 0.25 and iota < np.pi/2 - 0.1:
                    body.remove_legmotor()
                    body.add_legmotor(1)
                    leg_states[x] = "up"
                elif 0.75 >= outputs[0] > 0.5 and iota < np.pi/2 - 0.1:
                    body.remove_legmotor()
                    body.add_legmotor(2)
                    leg_states[x] = "up"
                elif 1 >= outputs[0] > 0.75 and iota < np.pi/2 - 0.1:
                    body.remove_legmotor()
                    body.add_legmotor(3)
                    leg_states[x] = "up"
                elif -0.5 <= outputs[0] < -0.25 and iota > (-np.pi/6 + 0.08):
                    body.remove_legmotor()
                    body.add_legmotor(-1)
                    leg_states[x] = "down"
                elif -0.75 <= outputs[0] < -0.5 and iota > (-np.pi/6 + 0.08):
                    body.remove_legmotor()
                    body.add_legmotor(-2)
                    leg_states[x] = "down"
                elif -1 <= outputs[0] < -0.75:
                    body.remove_legmotor()
                    body.add_legmotor(-1)
                    leg_states[x] = "down"
                else:
                ...
                # Body
                if 0.5 >= outputs[1] > 0.25 and delta > 0.42 +0.5:
                    body.remove_bodymotor()
                    body.add_bodymotor(-0.25)
                    body_states[x] = "backward"
                elif 0.75 >= outputs[1] > 0.5 and delta > 0.42 + 0.5:
                    body.remove_bodymotor()
                    body.add_bodymotor(-0.5)
                    body_states[x] = "backward"
                elif 1 >= outputs[1] > 0.75 and delta > 0.42 + 0.5:
                    body.remove_bodymotor()
                    body.add_bodymotor(-0.75)
                    body_states[x] = "backward"
                elif -0.5 <= outputs[1] < -0.25 and delta < 1.17 - 0.05:
                    body.remove_bodymotor()
                    body.add_bodymotor(-0.25)
                    body_states[x] = "forward"
                elif -0.75 <= outputs[1] < -0.5 and delta < 1.17 - 0.05:
                    body.remove_bodymotor()
                    body.add_bodymotor(-0.5)
                    body_states[x] = "forward"
                elif -0.5 <= outputs[1] < -0.25 and delta < 1.17 - 0.05:
...
if __name__ == '__main__':
    ...
    with open("winner.pkl", "wb") as f:
        ...
        f.close()
    plt.plot(range(100), max_angles)
    plt.rc('text', usetex=True)
    plt.rc('font', family='serif')
    plt.title(r"Max $\theta$ over Generations pop:25, no initial connection")
