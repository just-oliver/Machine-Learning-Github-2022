...
def eval_genomes(genome, config):
    ...
    while run:
        ...
        cooldown_leg = tok1 - tik1
        cooldown_body = tok1 - tik2
        for body in bodies:
        ...
        for x, body in enumerate(bodies):
            ...
            delta = angles[x][3]
            # leg inputs
            if outputs[0] > 0.5 and iota < np.pi/2 - 0.1 and cooldown_leg >= 0.25:
                body.remove_legmotor()
                body.add_legmotor(5)
                tik1 = time.perf_counter()
            elif outputs[0] < -0.5 and iota > (-np.pi/6 + 0.08) and cooldown_leg >= 0.25:
                body.remove_legmotor()
                body.add_legmotor(-5)
                tik1 = time.perf_counter()
            if 0.5 <= outputs[0] <= 0.5 and cooldown_leg >= 0.25:
                body.remove_legmotor()
                body.add_legmotor(0)
                tik1 = time.perf_counter()
            if iota >= np.pi/2 - 0.1:
                body.remove_legmotor()
                body.add_legmotor(0)
            elif iota <= (-np.pi/6 + 0.08):
                body.remove_legmotor()
                body.add_legmotor(0)
            # body inputs
            if outputs[1] > 0.5 and delta > 0.42 and cooldown_body >= 0.25:
                body.remove_bodymotor()
                body.add_bodymotor(-1)
                tik2 = time.perf_counter()
            elif outputs[1] < -0.5 and delta < 1.17 and cooldown_body >= 0.25:
                body.remove_bodymotor()
                body.add_bodymotor(1)
                tik2 = time.perf_counter()
            if 0.5 <= outputs[1] <= 0.5 and cooldown_body >= 0.25:
                body.remove_bodymotor()
                body.add_bodymotor(0)
                tik2 = time.perf_counter()
            if delta >= 1.17:
                ...
                body.add_bodymotor(0)
        # maximum = None
        for x, body in enumerate(bodies): #fitess function
            theta = angles[x][0]
            ge[x].fitness += abs(theta) 
        ...
        # pickle.dump(nets[max_index],open("best.pickle", "wb"))
